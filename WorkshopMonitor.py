...
async def CheckOne(modC, i):
    ...
    if int(epoch_time) != int(modC[1]):
        title = wdetails.json()["response"]["publishedfiledetails"][0]["title"]
        raw_tags = wdetails.json()["response"]["publishedfiledetails"][0]["tags"]
        ...
        config["userdata"].get("workshopid")[i] = {"id": f"{modC[0]}#{epoch_time}", "title": title, "tags": tags}
...
def GetNameForOne(id):
    ...
    wdetails = httpx.post(url, data=myobj)
    title = wdetails.json()["response"]["publishedfiledetails"][0]["title"]
    raw_tags = wdetails.json()["response"]["publishedfiledetails"][0]["tags"]
...
def CollectionToConfig(id):
    try:
        ...
        for item in modss:
            ...
            temp_title, temp_tags = GetNameForOne(temp)
            ids.append({"id": f"{temp}#000", "title": temp_title, "tags": temp_tags})
    ...
    async def my_background_task(self, LA):
        ...
        while not self.is_closed():
            try:
                ...
                print(
                    now.strftime(
                        f"{Fore.MAGENTA}[STOP] {Style.RESET_ALL}%H:%M:%S An error has accured current monitor cycle skipped!"
                    )
                )
...
async def help(interaction: nextcord.Interaction):
    embed = nextcord.Embed(
        title="Steam-Workshop-Monitor Help", url="https://github.com/UrekD/Steam-Workshop-Monitor", color=0xFF0000
    )
    embed.set_author(
        name="Steam-Workshop-Monitor Help",
        url="https://github.com/UrekD/Steam-Workshop-Monitor",
    ...
    embed.add_field(
        name="/remove mod#time",
        value="Removes specified mod from current configuration ex. /remove 1439779114#1531480087 ",
        inline=False,
    )
    embed.add_field(
        name="/add modid#000", value="Adds a mod to config in format modid#000, ex. /add 1439779114#000 ", inline=False
    )
    embed.add_field(name="/clear", value="Removes all mods from the config.", inline=False)
    embed.add_field(
        name="/fcheck",
        value="Force a check cycle indepentent of the interval.Force a check cycle indepentent of the interval",
        inline=False,
    )
    embed.add_field(
        name="/refill workshopid",
        value="Overwrites the config with the mods in workshop collection. ex, /refill 1332156191",
        inline=False,
    )
    ...
    if event.is_set() is False:
        try:
            ...
            if leng > 75:
                t = t * (leng / 50)
            data = [
                f"#{num} [{data[num]['title']}](https://steamcommunity.com/sharedfiles/filedetails/?id={data[num]['id']}) - {data[num]['tags']} | {data[num]['id']} "
                for num in range(0, leng)
            ]
            pages = menus.ButtonMenuPages(
                source=MyEmbedDescriptionPageSource(data), clear_buttons_after=True, timeout=t
            )
...
@bot.slash_command(
    name="remove", description="Removes specified mod from current configuration ex. /remove 1439779114#1531480087"
)
async def remove(interaction: nextcord.Interaction, arg: str):
    if event.is_set() is False:
        try:
            ...
            for idx, elem in enumerate(idsx):
                if elem["id"] == arg:
                    del idsx[idx]
...
@bot.slash_command(
    name="fcheck",
    description="Force a check cycle indepentent of the interval.Force a check cycle indepentent of the interval.",
)
...
async def add(interaction: nextcord.Interaction, arg: str):
    if event.is_set() is False:
        try:
            event.set()
            idsx = config["userdata"]["workshopid"]
            temp_title, temp_tags = GetNameForOne(arg)
            idsx.append({"id": f"{arg}#000", "title": temp_title, "tags": temp_tags})
            config["userdata"]["workshopid"] = idsx
            async with aiofiles.open("data/config.json", mode="w") as jsfile:
                await jsfile.write(json.dumps(config, indent=4, sort_keys=True))
...
@bot.slash_command(
    name="refill", description="Overwrites the config with the mods in workshop collection. ex, /refill 1332156191"
)
